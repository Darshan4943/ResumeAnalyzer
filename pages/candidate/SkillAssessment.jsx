...
function SkillAssessment() {
  ...
  useEffect(() => {
    setMainLoading(true);
    axios
      .get("http://192.168.1.208:2000/api/resume/skills/" + userDataGlobal?._id)
      .then((res) => {
  ...
  const toggleContent = () => {
    if (assesmentType !== "Normal" && !userDataGlobal.firstName) {
      ...
      if (
        ...
        axios
          .post("http://192.168.1.208:2000/api/qnaSkill", {
            skill: selectedSkill,
            level: level,
  ...
  const sumbit = () => {
    ...
    if (assesmentType === "Normal" ? questionIndex == 9 : questionIndex == 59) {
      axios
        .post("http://192.168.1.208:2000/api/assessment/add", {
  ...
  useEffect(() => {
    axios
      .get(
        `http://192.168.1.208:2000/api/assessment/getByUser/${userDataGlobal?._id}`
      )
      .then((res) => {
