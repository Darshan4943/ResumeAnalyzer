...
function OfferAccept() {
    ...
    const getOfferAcceptanceStatus = async () => {
        try {
            const response = await axios.get(`http://192.168.1.208:2000/api/preboarding/offerAcceptanceStatus/${applicantId}/${jobId}`);
    ...
    const OfferUpdate = async () => {
        ...
        try {
            const response = await axios.put(`http://192.168.1.208:2000/api/preboarding/offerAccept/${applicantId}/${jobId}`, { status })
            setLoading(false)
            setLoading1(false)
