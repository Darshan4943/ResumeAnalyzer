...
function Collection() {
  ...
  const getSkilotechCollectionData = () => {
    ...
    axios
      .get(
        `http://192.168.1.161:2000/api/folder/getSkilotechCollectionData/${userDataGlobal?._id}`
      )
      .then((res) => {
  ...
  const getTrashed = () => {
    setLoading(true);
    axios
      .get(`http://192.168.1.161:2000/api/folder/getTrashed/${userDataGlobal?._id}`)
      .then((res) => {
        setFolderList(res.data.data);
  ...
  const getUnSyncFiles = () => {
    ...
    axios
      .get(`http://192.168.1.161:2000/api/getUnsyncedFile/${userDataGlobal?._id}`)
      .then((res) => {
  ...
  const createFolder = () => {
    ...
    if (folderName?.length > 3) {
      ...
      axios
        .post("http://192.168.1.161:2000/api/folder/create", formData)
        .then((res) => {
          setRecall();
...
const addFiles = async () => {
        ...
        try {
          ...
          try {
            const response = await axios.post(
              "http://192.168.1.161:2000/api/folder/create",
              formData
            );
            setCount((prevCount) => prevCount + 1);
  ...
  //     const apiUrl = `http://192.168.1.161:2000/api/apiLogs/updateCollectionCount/${userDataGlobal?._id}`;
  ...
  const updateCollectionLimit = async () => {
    ...
    try {
      const apiUrl = `http://192.168.1.161:2000/api/apiLogs/updateCollectionCount/${userDataGlobal?._id}`;
      const anotherApiUrl = `http://192.168.1.161:2000/api/subscription/updateCollectionLimit/${userDataGlobal?._id}`;
