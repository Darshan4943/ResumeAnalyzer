...
function UploadDocuments() {
  ...
  const [previousStatus, setPreviousStatus] = useState("");
    ...
    const getDocumentsUploadStatus = async () => {
        try {
            const response = await axios.get(`http://192.168.1.161:2000/api/preboarding/documentsUploadStatus/${applicantId}/${jobId}`);
            setPreviousStatus(response.data.documentStatus)

      return response.data.documentStatus;
    } catch (error) {
      console.error(
        "Error fetching offer acceptance status:",
  ...
  const documentLabels = {
    isPhotoId: (
      <>
        Photo ID <span className="text-red">*</span>{" "}
    ...
    isAddress: (
      <>
        Address Proof <span className="text-red">*</span>{" "}
        <p className="text-[10px]">
    ...
    isPayroll: (
      <>
        Payroll Documents <span className="text-red">*</span>{" "}
        <p className="text-[10px]">(Pan Card, Bank statement)</p>
      </>
    ),
    isAcademic: (
      <>
        {" "}
        Academic Certificates<span className="text-red">*</span>
      </>
    ),
    isDegrees: (
      <>
        {" "}
        Degree Certificates<span className="text-red">*</span>
      </>
    ),
    isExperience: (
      <>
        {" "}
        Experience Letters<span className="text-red">*</span>
      </>
    ),
    ...
    const uploadDocuments = async () => {
        ...
        try {
            const response = await axios.post(
                `http://192.168.1.161:2000/api/preboarding/uploadDocuments/${applicantId}/${jobId}`,
                uploadedFiles, 
        ...
        {Object.keys(documentLabels).map((key) => {
          if (router.query[key] === "true") {
            return (
              <div
                key={key}
                className="flex flex-col gap-4 py-2 px-4 bg-white rounded-[12px] w-[350px] justify-between"
              >
                    ...
                    {files[key] ? (
                      <div className="w-full flex f-full ">
                        <div className="flex flex-row gap-[16px] items-center justify-between w-[95%]">
                          ...
                          <button
                            className="bg_Button sm:px-[8px] px-1 py-[6px] border border-[#06A9EF] rounded-[12px] text-[12px] sm:text-[16px] sm:min-w-[105px] min-w-[90px] cursor-pointer"
                        ...
                        <div className="flex flex-col items-center justify-center">
                          ...
                          >
                            <g clip-path="url(#clip0_6706_107586)">
                              ...
                              />
                              <path
                                d="M17.5 4H7.5C5.84315 4 4.5 5.34315 4.5 7V17C4.5 18.6569 5.84315 20 7.5 20H17.5C19.1569 20 20.5 18.6569 20.5 17V7C20.5 5.34315 19.1569 4 17.5 4Z"
                                stroke="#06A9EF"
                                stroke-width="2"
                                stroke-linecap="round"
                                stroke-linejoin="round"
                              />
                              <path
                                d="M4.5 15L8.5 11C8.95606 10.5611 9.47339 10.3301 10 10.3301C10.5266 10.3301 11.0439 10.5611 11.5 11L16.5 16"
                                stroke="#06A9EF"
                                stroke-width="2"
                                stroke-linecap="round"
                                stroke-linejoin="round"
                              />
                              <path
                                d="M14.5 14L15.5 13C15.9561 12.5611 16.4734 12.3301 17 12.3301C17.5266 12.3301 18.0439 12.5611 18.5 13L20.5 15"
                                stroke="#06A9EF"
                                stroke-width="2"
                                stroke-linecap="round"
                                stroke-linejoin="round"
      ...
      <div className="w-full flex justify-end mt-4 customMargins pb-6">
        {loading1 ? (
          ...
          <button
            ...
            className="bg-blue px-4 py-2 text-white rounded-[12px] flex justify-center items-center font-medium"
          >
            Submit
          </button>
        )}
        ...
        <>
          ...
          <div className="fixed z-[2000] top-0 left-0 right-0 bottom-0 flex items-center justify-center  ">
            <div className="customMargins w-full flex justify-center items-center ">
              <SuccessPopUp
                text={"Documents Already Submitted"}
                isButton={false}
              />
            </div>
          </div>
