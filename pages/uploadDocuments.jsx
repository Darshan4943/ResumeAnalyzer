...
function UploadDocuments() {
    ...
    const documentLabels = {
        isPhotoId: (
            <>
                Photo ID <p className="text-[10px]">(Aadhar card, Driving License, Pan Card, Passport, Voter ID Card)</p>
            </>
        ),
        isAddress: (
            <>
                Address Proof   <p className="text-[10px]">(Aadhar card, Driving License, Passport, Voter ID Card)</p>
            </>
        ),
        isPayroll: (
            <>
                Payroll Documents   <p className="text-[10px]">(Pan Card, Bank statement)</p>
            </>
        ),
    ...
    return (
        <>
            ...
            {updated ?
                ...
                :
                <>
                    <div className="flex customMargins gap-6 flex-wrap pt-6">
                        {Object.keys(documentLabels).map((key) => {
                            if (router.query[key] === "true") {
                                return (
                                    <div key={key} className="flex flex-col gap-4 p-4 bg-white rounded-[12px] w-[350px] justify-between">
                                        <p className="font-medium">{documentLabels[key]}</p>

                                        {loading[key] ? (
                                            <div className="border-dashed border-[3px] border-blue flex flex-col w-full rounded-[12px] px-[24px] py-[24px] items-center gap-[8px] upload-btn-wrapper min-h-[6rem]">
                                                <MiniLoader />
                                            ...
                                            <div
                                                ...
                                                onDrop={(e) => handleFileChange(e, key)}
                                                className="border-dashed border-[3px] border-blue flex flex-col rounded-[12px] px-[8px] py-[24px] items-center gap-[8px] upload-btn-wrapper min-h-[6rem] w-full bg-white"
                                            >
                                                    ...
                                                    <div className="w-full flex justify-center">
                                                        <div className="flex flex-row gap-[16px] items-center justify-between w-[95%]">
                                                            <div className="flex flex-row gap-[16px] items-center scr420:w-[60%] w-full">
                                                                ...
                                                                <span className="text-[12px] w-[80%] break-all ">{files[key].name}</span>
                                                            </div>
                                                            <button
                    ...
                    <div className="w-full flex justify-end mt-4 customMargins">
                        {loading1 ?
                            ...
                            :
                            <button onClick={uploadDocuments} className="bg-blue px-4 py-2 text-white rounded-[12px] flex justify-center items-center font-medium">
                                Submit
                            </button>
                        }
                    </div>
