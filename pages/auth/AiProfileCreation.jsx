...
function AiProfileCreation() {
  ...
  const navigate = () => {
    ...
    extracteText(file).then((result) => {
      if (result[0]?.text?.length > 0) {
        axios
          .post("http://192.168.1.208:2000/api/resume/extraction", {
            data: result,
          })
          .then((res) => {
  ...
  //               return axios.post("http://192.168.1.208:2000/api/resumeCheck", {
    ...
    <div className="customMargins py-[72px] flex gap-[96px] justify-center">
      ...
      <div className="bg-[#FFFFFF] rounded-[16px] p-6 flex flex-col gap-4">
        ...
        <div className="flex gap-6 justify-center">
          <button
            onClick={() => router.push("/auth/Sign_up?role=user")}
...
export default AiProfileCreation;
