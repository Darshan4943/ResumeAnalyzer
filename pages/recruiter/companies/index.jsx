...
function Index() {
  ...
  const fetchCompanyData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:2000/api/company/getCompaniesById/${id}`);
  ...
  const handleDelete = async (companyId) => {
    ...
    try {
      setLoading(true);
      const response = await axios.delete(`http://localhost:2000/api/company/deleteCompany/${companyId}`);
