...
import PurchasedSuccessful from "../components/models/purchasedSuccessfull";
import PurchasedFailed from "../components/models/purchasedFailed";
...
function Payment() {
  ...
  useEffect(() => {
    ...
    if (skilotechCollection) {
      localStorage.setItem("skilotechCollection", skilotechCollection);
    }
    if (myCollection) {
      localStorage.setItem("myCollection", myCollection);
    }
  }, [id, application, skilotechCollection, myCollection]);
  ...
  //     });
  //     axios
  //       .post("https://api.skilotech.com/api/add/subscription", {
  ...
  const getPriceId = async (obj) => {
    ...
    if (currency) {
      const { data } = await axios.post(
        "https://api.skilotech.com/api/getPriceId",
        {
    ...
    if (!data.checked) {
      ...
      if (found.length > 0) {
        ...
        try {
          ...
          axios
            .post("https://api.skilotech.com/api/proceed/resumePayment", {
              priceId,
            })
            .then((res) => {
              if (res.data.success) {
    ...
    () => async (storedId) => {
      try {
        ...
        const response = await axios.get(
          "https://api.skilotech.com/api/retrieve/session",
          {
            params: { storedId },
          }
        );
  ...
  const handlePaidSession = async (session) => {
    ...
    const skilotechCollection = localStorage.getItem("skilotechCollection");

    try {
      if (skilotechCollection) {
        await axios.put(
          `https://api.skilotech.com/api/updateResume/paymentStatus/${storedId}`
        );
      } else if (myCollection) {
        await axios.put(
          `https://api.skilotech.com/api/updateCollection/paymentStatus/${storedId}`
        );
      } else if (application) {
        await axios.put(
          `https://api.skilotech.com/api/updateApplication/paymentStatus/${storedId}`
        );
      } else {
        await axios.put(
          `https://api.skilotech.com/api/user/updateUserPaymentStatus/${userDataGlobal?._id}`
        );
      }
  ...
  const handleFreeSession = async () => {
    ...
    if (!data.checked) {
      ...
      if (found.length > 0) {
        ...
        setLoading(true);
        try {
          await axios.post("https://api.skilotech.com/api/add/subscription", {
            userId: userDataGlobal?._id,
  ...
  return (
    <>
      {isLogin && userDataGlobal?.role !== "user" && (
        ...
        >
          <div
            ...
            className="customMargins py-3 flex justify-between items-center cursor-pointer"
          >
            <img
      ...
      <div
        className={
          " w-full plan-container  flex justify-center items-center py-12 customMargins "
        }
      >
        {successModel.visible && (
          <div className="expiryModel">
            <div className="modal">
              <div className="bg-white p-[16px] rounded-[18px] w-[400px] min-h-[200px] flex flex-col gap-6 items-center justify-center">
                {loading ? (
                  <>
                    <div role="status">
                      <svg
                        aria-hidden="true"
                        className="w-[40px] h-[40px] text-[#e0e0e0] animate-spin fill-[#06a9ef]"
                        viewBox="0 0 100 101"
                        fill="none"
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-[16px]">
                      <div className="flex flex-col gap-[12px]  items-center">
                        ...
                        />
                        <span className="text-[28px] text-[#333333] font-semibold text-center">
                          Payment Successful
                        </span>
                      </div>
                    ...
                    <div className="flex flex-row justify-center items-center gap-[16px] ">
                      <button
                        ...
                        className="border-[#06A9EF]  px-[24px] bg_Button h-[40px] rounded-[30px] "
                      >
                        Done
                      </button>
                    </div>
          ...
          <div className=" flex flex-col gap-4 justify-center w-[100%] ">
            <div className="text-[16px] font-[600]  flex gap-2 items-center">
              <svg
                ...
                xmlns="http://www.w3.org/2000/svg"
              >
                <g mask="url(#mask0_5959_80298)">
                  <path
                    d="M9.12705 15.1641L15.6604 21.6974L13.9979 23.3307L4.66455 13.9974L13.9979 4.66406L15.6604 6.2974L9.12705 12.8307H23.3312V15.1641H9.12705Z"
                    fill="#1C1B1F"
                  />
                </g>
              </svg>
