...
function Payment() {
    ...
    const { success, cancelled, id,isApplication,isResume } = router.query;
    ...
    useEffect(() => {
        ...
        if (isApplication) {
          localStorage.setItem("isApplication", isApplication);
        }
        if (isResume) {
          localStorage.setItem("isResume", isResume);
        }
      }, [id, isApplication, isResume]);
    ...
    const handlePaidSession = async (session) => {
        ...
        try {
            await axios.put(`http://localhost:2000/api/update/paymentStatus/${storedId}`);
            // setPurchaseCount(1)
