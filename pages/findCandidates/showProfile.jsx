...
function ShowProfile() {
    ...
    const currentJob = userData?.workExperiance?.find((exp) => exp.isCurrent);

    const topEdu = userData?.education?.[0];
    const keySkills = userData?.skills?.map((s) => s.label).join(" | ");
    const locations = userData?.jobPrefrences?.preferedLocation?.map((l) => l.location).join(", ");
            ...
            <div className='flex flex-col gap-4'>
                <div className="border border-[#DEDEDE] rounded-lg p-4  gap-4 shadow-sm bg-white flex justify-between">
                    {userData?.profilePicture?.img ?
                        <img
                            src={userData?.profilePicture?.img}
                            alt="Profile"
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-lg font-medium">
                            {/* <input
                                    ...
                                    /> */}
                            <div onClick={() => router.push(`/findCandidates/showProfile?id=${userId}`)} className=" cursor-pointer">
                        ...
                        <div className="text-[12px] text-gray-600 flex items-center gap-2 mt-1">
                            ...
                            {getTotalExperience(userData)} &nbsp;{currentJob?.currentCTC && `| ₹${currentJob.currentCTC}`}
                    ...
                    <div className="flex justify-between gap-4">
                        ...
                        <div className="flex flex-col gap-4">
                            <button
                                disabled={!userData?.resumeUrl}
                                onClick={() => save(userData)}
                                className={`p-2 rounded-full ${userData?.resumeUrl && "hover:bg-[#E9EEF6] hover:fill-black transition-colors cursor-pointer"}`}
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    height="24px"
                                    ...
                                    width="24px"
                                    className={`fill-[#8993a4] ${userData?.resumeUrl && "hover:fill-black"}`}
                                >
                ...
                <div className='bg-white rounded-[12px] p-4'>
                    <div className=" flex flex-wrap gap-2">
                        ...
                        {userData?.skills.map((s, i) => (
                            <span key={i} className="bg-[#F4F5F7]  text-[12px] font-medium p-1 rounded-[4px]">
                                {s.label}
