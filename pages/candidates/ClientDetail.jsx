...
function ClientDetail({ tabIndex }) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("http://192.168.1.208:2000/api/cover/get/" + clientId)
      .then((res) => {
  ...
  const deleteResume = () => {
    ...
    axios
      .delete("http://192.168.1.208:2000/api/resume/deleteResume", {
        data: { ids },
      })
      .then((response) => {
  ...
  const deleteCoverLetter = () => {
    ...
    axios
      .delete(`http://192.168.1.208:2000/api/cover/delete/${ids}`)
      .then((response) => {
        toast.success("Resume Deleted successfully");
