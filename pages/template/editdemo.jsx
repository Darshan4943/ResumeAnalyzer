...
const handleKeyDown = (e) => {
  if (e.key === "Backspace") {
    ...
    // Get leaf and offset at the current selection index - 1 (character before cursor)
    const [leaf, offset] = quill.getLeaf(selection.index - 1);
    if (!leaf) return;

    const blot = leaf.parent;

    // Check if the blot is a TokenBlot (custom-token)
    if (blot && blot.domNode?.classList?.contains("custom-token")) {
      e.preventDefault();

      // Delete the entire blot by removing from blot.offset() to blot.offset() + blot.length()
      const blotIndex = blot.offset(quill.scroll);
      const blotLength = blot.length();

      quill.deleteText(blotIndex, blotLength);

      // Set cursor at blotIndex after deletion
