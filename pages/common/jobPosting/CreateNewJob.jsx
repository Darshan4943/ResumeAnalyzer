...
function CreateNewJob() {
  ...
  const fetchCompanyDetails = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `https://api.skilotech.com/api/getEmployerCompanies/${userDataGlobal?.companyId}`
      );
  ...
  const getcompaniesdetails = (id) => {
    ...
    axios
      .get(`https://api.skilotech.com/api/company/fetchCompaniDetails/${id}`)
      .then((res) => {
        const {
          companyName,
          companyLogo: logo,
  ...
  const getJobDetails = async (reqId) => {
    try {
      setLoading(true);
      const response = await axios.get(
        `https://api.skilotech.com/api/getRequisitionById/${reqId}`
      );
                    ...
                    <div className="grid grid-cols-1 scr500:grid-cols-10 md:grid-cols-11 gap-[16px] p-[10px] md:p-[16px] w-full">
                      <div className="flex flex-col gap-[8px] col-span-1 scr500:col-span-5 md:col-span-3 justify-between">
                        ...
                        {userDataGlobal?.role === "employer" ? (
                          <div
                            className={`w-full border ${formError.companyName
                              ? "border-red"
                              : "border-[#DEDEDE]"
                              } h-[42px] rounded-[8px] flex items-center px-2 py-2 text-[12px] text-[#000] bg-[#f5f5f5]`}
                          >
                            {camelCase(
