...
const ChatBox = ({
  ...
  //   axios
  //     .put(
  //       `https://api.skilotech.com/api/subscription/updateChatLimit/${userDataGlobal?._id}`
  //     )
  ...
  const submitHandler = (e) => {
    ...
    if (text?.length > 5) {
      ...
      setLoading(true);
      axios
        .post("https://api.skilotech.com/api/qna", {
          question: text,
          lastQuestion: chat.slice(chat.length - 5, chat.length),
  ...
  const extracteText = async (file) => {
    return new Promise(async (resolve, reject) => {
      ...
      //       "https://api.skilotech.com/convert",
      //       formData,
      //       {
      //         responseType: "blob",
      //       }
  ...
  const handleFile = (e) => {
    ...
    if (file?.type == "image/png" || file?.type == "image/jpeg") {
      ...
      axios
        .post("https://api.skilotech.com/api/getImageUrl", formData)
        .then((res) => {
          if (res.data.success) {
