...
const ApplicantRanking = ({ applications, jobPost, loadingg }) => {
  const [selectedFile, setSelectedFile] = useState(null);
  const [showDetails, setShowDetails] = useState(false);
  const [resumeList, setResumeList] = useState([]);
  console.log(33434, applications);
  useEffect(() => {
    if (applications) {
      let getRankingResumes = applications.filter(
  ...
  console.log(47674, resumeList);

  const handleDownLoad = () => {
    window.location.href = applicant.resumeUrl;
  };
