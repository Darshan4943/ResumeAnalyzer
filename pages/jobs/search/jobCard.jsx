...
const JobCard = ({ data, setJd, resume, jd }) => {
 ...
 const { profileData } = useSelector((state) => state.profile.profileData);         const { userDataGlobal } = useSelector((state) => state.user.userData);
  ...
  const getData = () => {
    ...
    if (data) {
      axios
        .get("http://192.168.1.161:2000/api/job/getById/" + data?._id)
        .then((res) => {
          setLoading(false);
          setIsApplied(
  ...
  const applyForJob = (data) => {
    setLoading(true);
    axios
      .post("http://192.168.1.161:2000/api/job/apply/" + data?._id, {
  ...
  const SaveJob = (id) => {
    axios
      .post(`http://192.168.1.161:2000/api/saveJob/${userDataGlobal?._id}/${id}`)
      .then((res) => {
