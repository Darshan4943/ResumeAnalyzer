...
function ApplyForm() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://192.168.1.208:2000/api/userJobDetails/getUserJobDetailsById/${userDataGlobal?._id}`
      )
      .then((res) => {
  ...
  const updateApplyCount = () => {
    axios
      .put(
        `http://192.168.1.208:2000/api/subscription/updateApplyLimit/${userDataGlobal?._id}`
      )
      .then((res) => {})
      .catch((err) => console.error(err));
  };
  ...
  const applyForJob = () => {
    ...
    axios
      .post(`http://192.168.1.208:2000/api/job/apply/${id}`, formDataToSend, {
        headers: {
      ...
      .finally(() => {
        axios
          .post(
            "http://192.168.1.208:2000/api/userJobDetails/createOrUpdateUserJobDetails",
            {
