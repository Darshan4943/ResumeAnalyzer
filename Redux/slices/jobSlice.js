...
export const fetchAppliedJob = createAsyncThunk('job/fetchAppliedJob', async (userId) => {
    const response = await axios.get(`http://192.168.1.161:2000/api/job/getAppliedJobs/${userId}`);
  ...
  export const fetchSavedJobIds = createAsyncThunk('job/fetchSavedJobIds', async (userId) => {
    const response = await axios.get(`http://192.168.1.161:2000/api/job/getSaveJobIds/${userId}`);
