...
export const Api = ({ }) => {
  ...
  //   axios
  //     .put("http://192.168.1.208:2000/api/skiloteckuser/clrLocalStorage/" + userDataGlobal?._id)
  //     .then((res) => {
  ...
  useEffect(() => {
    axios
      .get("http://192.168.1.208:2000/api/plans/getAllPlans")
      .then((res) => {
  ...
  //       axios
  //         .get("http://192.168.1.208:2000/api/skiloteckuser/user/" + decoded._id)
  //         .then((res) => {
  ...
  useEffect(() => {
    ...
    if (userDataGlobal) {
      axios
        .get("http://192.168.1.208:2000/api/subscription/" + userDataGlobal?._id)
        .then((res) => {
          ...
          if (result?.isActive == true) {
            ...
            if (timezone >= newEnddate && result.isActive) {
              axios
                .put(
                  "http://192.168.1.208:2000/api/subscription/update/" + result._id
                )
                .then((res) => {
                  if (res.data.success) {
  ...
  useEffect(() => {
    if (userDataGlobal?._id) {
      ...
      axios
        .post("http://192.168.1.208:2000/api/apiLogs/get", {
          userId,
          role,
        })
        .then((res) => {
    ...
    const processCountryData = async (results) => {
      ...
      if (countryData) {
        ...
        const exchangeRate = await axios.get(
          `http://192.168.1.208:2000/api/exchangeRate/${currency}`
        );
