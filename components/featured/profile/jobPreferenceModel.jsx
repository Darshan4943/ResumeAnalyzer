...
const JobPrefrenceModal = ({ setEditView }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const obj = {
      ...
      shift: data.shift,
      preferedLocation: preferedLocation.map((item) => ({
    ...
    axios
      .put(
        "https://dev.api.skilotech.com/api/candidate/updateJobPreferance/" +
          userDataGlobal?._id,
        obj
      )
      .then((res) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 rounded-[16px] max-h-[calc(100vh-140px)] modal_container py-3 px-6 overflow-y-auto ">
        <div className="flex flex-col gap-1">
          <div className="modal_title flex justify-between gap-[16px]  items-center">
            <p className="text-[18px] font-[600] min-w-[200px]">
              {" "}
          ...
          <p className="text-[12px]">
            Customize your job settings to match what youre looking for in a
            job.
          </p>
        </div>
        <form action="">
          <div className="w-full form_container_col form_container">
            ...
            <div className="input-container w-[100%] md:w-[46%] ">
              ...
              <ReactSelect
                ...
                className="w-full text-[12px] font-[400] outline-none border-[#DEDEDE]"
                onChange={(data) => {
                  const isDuplicate = preferedLocation.some(
                    (location) => location.location === data.value
                  );
                  ...
                  if (!isDuplicate) {
                    ...
                    toast.warn("This location is already selected.");
                  }
                }}
              />
            </div>
          ...
          <div className="location-list ">
            {preferedLocation.map((item, index) => (
              <div
                key={index}
                className="location_item text-[12px] font-[400] h-[36px]"
              >
                {item.location}
