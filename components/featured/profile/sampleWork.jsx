...
function SampleWork({ setaddSampleWork, Project, editProject }) {
  ...
  const handleSubmit = () => {
    ...
    if (isEditing) {
      axios
        .put(
          `https://dev.api.skilotech.com/api/candidate/${userDataGlobal?._id}/updateProject/${Project._id}`,
          projectData
        )
        .then((res) => {
      ...
      axios
        .post(
          `https://dev.api.skilotech.com/api/candidate/addProject/${userDataGlobal?._id}`,
          projectData
        )
        .then((res) => {
          dispatch(fetchUserData());
