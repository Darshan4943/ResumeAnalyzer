...
function HonorsAwards({ setAddAchivements, editAchievement, Achievement }) {
  ...
  const handleSubmit = () => {
    ...
    if (isEditing) {
      axios
        .put(
          `https://api.skilotech.com/api/candidate/${userDataGlobal?._id}/updateAchivement/${Achievement._id}`,
          awardData
        )
        .then((res) => {
      ...
      axios
        .post(
          `https://api.skilotech.com/api/candidate/addAchivement/${userDataGlobal?._id}`,
          awardData
        )
        .then((res) => {
          dispatch(fetchUserData());
