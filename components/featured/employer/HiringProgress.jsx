...
function HiringProgress({ hiringData = [] }) {
        ...
        <div className='flex flex-col gap-6 overflow-y-auto scr1024:px-6 px-2'>
            {hiringData.length > 0 ? (
                hiringData.map((level, index) => (
                    <div key={level._id} className='flex gap-4'>
                        ...
                        <div className='flex flex-col gap-4 w-full'>
                                ...
                                <div className='flex flex-col gap-1'>
                                    <div className='flex gap-4 text-[16px] font-medium'>
                                        ...
                                        <div className='flex gap-2'>
                                            ...
                                            <p className='font-normal'>{level.score}</p>
                                        </div>
                                    </div>
                                    <p className={`text-[14px] font-medium ${level.status === "Verified" ? "text-[#0C8A0A]" : "text-red"}`}>{level.status}</p>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className='px-9 py-3 bg-[#06A9EF] rounded-[30px] min-w-[240px] w-[30%] text-[16px] font-semibold text-white'
                            >
                                Move to Next Stage
                            </button>
                        </div>
