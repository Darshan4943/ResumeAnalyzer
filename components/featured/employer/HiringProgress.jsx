...
import { Close_svg, DummyProfileSvg } from "../../../utils/svg";
      ...
      {isDetails &&
        <>
          ...
          <div className="fixed z-[2000] top-0 left-0 right-0 bottom-0 flex items-center justify-center">
            <div className="bg-white rounded-[16px] w-[500px] p-6 flex flex-col gap-4">
              ...
              <div className="flex gap-4 justify-between items-center">
                ...
                <div onClick={() => setIsDetails(false)} className=" cursor-pointer">
                  <Close_svg />
                </div>

              </div>
              <p className="text-[16px] font-semibold">{viewLevel?.title}</p>
              <div className="grid grid-cols-12">
                {viewLevel?.interviewDate && (
                  <div className="flex flex-col gap-2 col-span-4">
                    <p className="text-[#646464] text-[14px] font-normal">
                      Interview Date
                    </p>
                    <div>
                      {formatInterviewDate(viewLevel?.interviewDate)}
                    </div>
                  </div>
                )}
                {viewLevel?.assignOn && viewLevel?.isTask && (
                  <div className="flex flex-col gap-2 col-span-4">
                    <p className="text-[#646464] text-[14px] font-normal">
                      Assign On
                    </p>
                    <div>
                      {formatInterviewDate(viewLevel?.assignOn)}
                    </div>
                  </div>
                )}
                {viewLevel?.score &&
                  <div className="flex flex-col gap-2  col-span-4">
                    <p className="text-[#646464] text-[14px] font-normal">
                      {viewLevel?.isTask ? "Task" : "Interview"} Score
                    </p>
                ...
                <div className="flex flex-col gap-2 col-span-4">
                  <p className="text-[#646464] text-[14px] font-normal">
                    {viewLevel?.isTask ? "Task" : "Interview"} Status
                  </p>
                  <div
                    className={`px-[10px] py-[3px] rounded-[80px] w-fit ${viewLevel?.status === "Pending"
                      ? "text-[#FFB836] bg-[#EB85331A]"
                      : viewLevel?.status === "Conducted" ||
                        viewLevel?.status === "Completed"
                        ? "text-[#0C8A0A] bg-[#E2FFE1]"
                        : "text-[#FF6550] bg-[#FF65501A]"
                      }`}
                  >
                    {viewLevel?.status === "Pending"
                      ? "In Progress"
                      : viewLevel?.status}
                  </div>
                </div>
              ...
              {!viewLevel?.isOnline && viewLevel?.isInterview && (
                <div className="flex flex-col gap-2 ">
                  <p className="text-[#646464] text-[14px] font-normal">
                    Interview Location
                  </p>
                  <div>{viewLevel?.interviewLocation}</div>
                </div>
              )}
              {viewLevel?.isOnline && viewLevel?.isInterview && (
                <div className="flex flex-col gap-2 ">
              ...
              <div className="flex gap-6">
                {(
                  <div className="flex flex-col gap-2">
                    <p className="text-[#646464] text-[14px] font-normal">
                      {viewLevel?.isTask ? "Reviewer" : "Conducted by"}
                    </p>
                    <div className="flex gap-4 flex-wrap">
                      {(viewLevel?.isTask
                        ? viewLevel?.taskReviewer
                        : viewLevel?.interviewer
                      )?.map((assignee, i) => (
              ...
              <div className="h-[1px] w-[100%] bg-[#D6DDEB]"></div>
                    ...
                    <div className="flex flex-col gap-4 w-full  -mt-1">
                      ...
                      {(index === 0 && applicantDetails.hiringStage !== "Rejected" && !clientView) && (
                        <button
                          ...
                          className="px-6 h-[38px] bg-[#06A9EF] rounded-[30px] min-w-[190px] w-[190px] text-[14px] font-semibold text-white"
                        >
                          Move to Next Level
                        </button>
                      )}
                          ...
                          <div className="flex flex-col gap-4 w-full">
                            <div className="flex gap-4 justify-between ">
                              ...
                              <div className=" flex justify-end">
                                <button onClick={() => viewDetaiils(level)} className="text-blue text-14px font-medium underline ">
                                  See Details
                                </button>
                              </div>
                            ...
                            <div className="grid grid-cols-12 gap-y-4">
                              ...
                              {level?.isOnline && level?.isInterview && (
                                <div className="flex flex-col gap-2 col-span-4">
                                  <p className="text-[#646464] text-[14px] font-normal">Meeting Link</p>
                                  <div>
                                    {level?.meetingLink?.length > 25
                                      ? level?.meetingLink.substring(0, 25) + "..."
                                      : level?.meetingLink}
                                  </div>
                                </div>
                              )}
                              ...
                              {level?.comment && (
                                <div className="flex flex-col gap-2 w-full col-span-8">
                                  <p className="text-[#646464] text-[14px] font-normal">Comment</p>
                                  {level?.comment?.length > 60
                                    ? level?.comment.substring(0, 60) + "..."
                                    : level?.comment}
                                </div>
                              )}
                              <div className={`flex flex-col gap-2 ${level?.comment || !level?.meetingLink ? "col-span-12" : "col-span-8"}`}>
                            ...
                            <div className="flex gap-4 justify-end ">
                              ...
                              {(level?.status === "Pending" && level?.isInterview) &&
                                <button
                                  onClick={() => {
                                  ...
                                  className="px-6 h-[38px] border border-[#06A9EF] rounded-[30px] min-w-[210px] w-[210px] text-[14px] font-semibold "
                                >
