  ...
  const hiredCandidate = async (applicantId, jobId) => {
    ...
    try {
      const response = await axios.put(
        `https://api.skilotech.com/api/preboarding/hiredCandidate/${applicantId}/${jobId}`
      );
        ...
        <div className="grid grid-rows-1 w-full ">
          <div className="grid grid-cols-6 w-full grid-flow-col">
            {labels.map((req, index) => (
              <div
                key={index}
                className={`flex px-4 text-[14px] font-[600] font-Montserrat text-[#333333] py-4 items-center bg-[#EFFAFF] w-full ${
                  index === labels.length - 1
                    ? "justify-end"
                    : "justify-between"
                }`}
              >
                <p className={index === labels.length - 1 ? "text-right" : ""}>
                  {req}
                </p>
              </div>
            ))}
