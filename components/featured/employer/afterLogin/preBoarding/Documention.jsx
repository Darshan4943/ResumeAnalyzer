...
const Documention = ({ toggleContentt, setToggle }) => {
  ...
  const reminder = async (applicants) => {
    setLoadingApplicantId(applicants._id);
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `https://api.skilotech.com/api/getInPreboadingCandidates/${userDataGlobal?._id}`,
        {
          params: {
            page: page,
            limit: limit,
  ...
  const moveToVerification = async (applicantId, jobId) => {
    try {
      const response = await axios.put(
        `https://api.skilotech.com/api/preboarding/moveToVerification/${applicantId}/${jobId}`
      );
                      ...
                      >
                        <div className="grid grid-cols-6 w-full ">
                          <div className="flex items-center justify-start col-span-1">
                            <div onClick={() => router.push(`/common/hiring/ApplicantDetails?applicantId=${applicants?._id}&id=${applicants?.jobId}`)} className="flex justify-start text-[14px] font-[600] items-center cursor-pointer  gap-1 scr1024:gap-[16px]">
