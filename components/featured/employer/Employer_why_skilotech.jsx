import React from "react";

function Employer_why_skilotech() {
  return (
    <div className="flex justify-center items-center  customMargins text-center ">
      <div className="p-6 justify-center items-center gap-4 rounded-lg bg-[#024e6f] text-white font-montserrat text-[36px] font-semibold leading-12">
        <span className="text-[#ffda1d] text-center font-montserrat text-[36px] font-semibold leading-12">
          Revolutionize
        </span>{" "}
        your hiring process with our cutting-edge platform designed for
        recruiting{" "}
        <span className="text-[#ffda1d] text-center font-montserrat text-[36px] font-semibold leading-12">
          skilled workers
        </span>
      </div>
    </div>
  );
}

export default Employer_why_skilotech;
