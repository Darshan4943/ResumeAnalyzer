      ...
      <View style={{ width: 595, minHeight: 840, paddingHorizontal: 20, paddingVertical: 10, gap: 15, flexDirection: 'column', alignContent: 'center', justifyContent: 'start' }} >
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: 550, gap: 25 }}>
          <View style={{ width: 173, flexDirection: 'column', gap: 20 }}>
            <View style={{ width: 140, gap: 12 }}>
              <View style={{ maxWidth: 140 }}>
                <Text style={{ fontSize: 28,  fontFamily:'Montserrat 700', }}>{data.firstName}</Text ><Text style={{ fontSize: 28, fontFamily:'Montserrat 700', }}>{data.lastName}</Text>
              </View>
              <Text style={{ fontSize: 12,fontFamily:'Inter 400',maxWidth: 140 }}>{data.designation}</Text>
            </View>
            <View style={{ flexDirection: 'column', gap: 14, width: 130, gap: 4 }}>
              <View style={{ maxWidth: 120, gap: 6, flexDirection: "row", alignItems: "center", }}>
                ...
                />
                <Text style={{ fontSize: 10, fontFamily:'Inter 400', color: "#282627" }}>{data.mobileNumber}</Text>
              </View>
              ...
              <View style={{ maxWidth: 120, gap: 6, flexDirection: "row", alignItems: "center", }}>
                <Image
                  ...
                  style={{ width: 16, height: 16, }}
                />
                <Text style={{ fontSize: 10,fontFamily:'Inter 400',color: "#282627" }}>{data.location}</Text>
              </View>
          ...
          <View style={{ height: 170, width: 173, flexDirection: 'column', gap: 4, alignItems: 'start', justifyContent: 'flex-start' }}>
            <Text style={{ fontSize: 12, fontFamily:'Inter 400', color: "#282627" }}>About</Text>
            <Text style={{ fontSize: 10, fontFamily:'Inter 400', color: "#6D6E71", maxWidth: 170 }}>{data.summery}</Text>
        ...
        <View style={{ gap: 13, flexDirection: 'row', width: 550 }}>
          <View style={{ width: 170, gap: 4 }}>
            <Text style={{ fontSize: 12, fontFamily:'Inter 400', }}>LANGUAGE</Text>
                    ...
                    return (
                      <View style={{ width: "100%", flexDirection: "column" }} key={index}>
                        <View style={{ flexDirection: "row", alignItems: "center" }}>
                          <Text style={{ color: "#414142", fontSize: 12, width: 60, fontFamily:'Inter 400',fontWeight: "light" }}>
                            {detail.languages}
                          </Text>
                ...
                return (
                  <View style={{ paddingRight: 4, flexDirection: "column" }} key={index}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                      <Text style={{ color: "#414142", fontSize: 12, width: 80, fontFamily:'Inter 400', }}>
                        {detail.skill}
                      </Text>
          ...
          <View style={{ width: 170, gap: 4 }}>
            <Text style={{ fontSize: 12, fontFamily:'Inter 400', }}>SOFTWARE</Text>
            {/* <View style={{ flexDirection: "column", width: 170, gap: 6 }}>
                  {data.skills?.map((detail, index) => {
                    const calculateWidthPercentage = (rating) => {
                      let ratingPercentage = 0;
        ...
        <View style={{ width: 550, flexDirection: 'row', gap: 20 }}>
          <View style={{ width: 150, flexDirection: 'column', gap: 12 }}>
            <View style={{ gap: 10, flexDirection: 'column' }}>
              <Text style={{ fontSize: 12,fontFamily:'Inter 400', }}>
                EDUCATION
              </Text>
              {
                  ...
                  <>
                    <View style={{ flexDirection: 'column', gap: 4, maxWidth: 150 }}>
                      ...
                      <View style={{ gap: 8, flexDirection: 'column', justifyContent: 'start', alignItems: 'start' }}>
                        <Text style={{ fontSize: 12, fontFamily:'Inter 400', color: '#282627', maxWidth: 150 }}> {detail.qualification}</Text>
                        <Text style={{ fontSize: 12, fontFamily:'Inter 400', color: '#6D6E71', maxWidth: 150 }}> {detail.instituteName}</Text>
                        <Text style={{ fontSize: 12, fontFamily:'Inter 400', color: '#6D6E71', maxWidth: 150 }}>{detail.specialization}</Text>
                      </View>
                    </View>
            ...
            <View style={{ flexDirection: 'column', maxWidth: 150, paddingTop: 10 }}>
              <Text style={{ fontFamily:'Inter 400', fontSize: 12, color: '#282627' }}>HOBBIES</Text>
              {data.hobbies?.map((detail, index) => (
                <View key={index} style={{ flexDirection: "column", }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                    <Text style={{ color: "#282829", }}>&#8226;</Text>
                    <Text style={{ fontFamily:'Inter 400',color: "#5E5F5E", fontSize: 10 }}>{detail.title}</Text>
                  </View>
                </View>
              ))}
            </View>
            ...
            {/* AWORD  */}
            <View style={{ flexDirection: 'column', maxWidth: 150, paddingTop: 10 }}>
              <Text style={{ fontFamily:'Inter 400', fontSize: 12, color: '#282627' }}>AWARDS</Text>
              {/* {data.hobbies?.map((detail, index) => (
        <View key={index} style={{ flexDirection: "column", }}>
          <View style={{ flexDirection: "row", alignItems: "center",gap:4  }}>
            <Text style={{  color: "#282829", }}>&#8226;</Text>
          ...
          <View style={{ flexDirection: 'column', width: 400, gap: 12 }}>
            <Text style={{ fontSize: 12,fontFamily:'Inter 400',}}> EXPERIENCE</Text>
            <View style={{ width: 40 }}>
              <View>
                    ...
                    <>
                      <View style={{ width: 340, flexDirection: 'row', gap: 10 }}>
                        <View style={{ width: 40 }} >
                          ...
                          <Text style={{ fontSize: 14, fontWeight: "normal",fontFamily:'Inter 400', color: '#282627' }}>
                            {" "}
                        ...
                        <View style={{ gap: 8, flexDirection: 'column' }}>
                          <View style={{ gap: 4, flexDirection: 'row' }}>
                            <Text style={{ fontSize: 14, fontFamily:'Inter 400', color: '#2C363D' }}>     {detail.organization} -    {detail.designation}</Text>

                          </View>
                          <Text style={{ fontSize: 10, fontFamily:'Inter 400', color: '#6D6E71' }}> {detail.description}</Text>
                        </View>
                      </View>
