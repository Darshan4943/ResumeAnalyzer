    ...
    <Page size="A4">
      <View style={{ width: 595, color: "#FFFFFF", paddingTop: 32, paddingLeft: 42, paddingRight: 42, gap: 24, display: "flex", flexDirection: "row" }}>
        ...
        <View style={{ width: 164, gap: 26 }}>
          ...
          <View style={{ display: "flex", flexDirection: "column", gap: 6, backgroundColor: "#414042", paddingTop: 88, paddingHorizontal: 16, marginTop: 72, paddingBottom: 16 }}>
            <View style={{ display: "flex", flexDirection: "row", gap: 6, width: "95%", flexWrap: "wrap" }}>
              ...
              <Text style={{ fontWeight: 400, fontSize: 12, color: "#FFFFFF" }}>{data.mobileNumber}</Text>
            </View>
            <View style={{ display: "flex", flexDirection: "row", gap: 6, width: "95%" }}>
              ...
              <Text style={{ fontWeight: 400, fontSize: 12, color: "#FFFFFF" }}>{data.email}</Text>
            </View>
            ...
            <View style={{ display: "flex", flexDirection: "row", gap: 6, width: "95%" }}>
              ...
              <Text style={{ fontWeight: 400, fontSize: 12, color: "#FFFFFF" }}>{data.location}</Text>
            </View>
          </View>
          ...
          <View>
            ...
            <View style={{ flexDirection: "column", gap: 12 }}>
              <View>
                <Text style={{ fontWeight: 400, fontSize: 16, color: "#414042" }}>Education</Text>
              </View>
              {data?.education?.map((detail, index) => (

                <View style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <View>
                    <Text style={{ fontWeight: 700, fontSize: 10, color: "#414042" }}>{detail.qualification} - {detail.specialization} </Text>
                    <Text style={{ fontWeight: 700, fontSize: 10, color: "#414042" }}>{detail.instituteName}</Text>
                  </View>

                  <View style={{ height: 1, width: 23, backgroundColor: "#414142" }}></View>
                  <View><Text style={{ fontWeight: 400, fontSize: 10, color: "#414042" }}>{detail.duration?.start?.year}-{detail.duration?.end?.year}</Text></View>
          ...
          <View style={{ flexDirection: "column", gap: 12 }}>
            ...
            <Text style={{ fontWeight: 400, fontSize: 16, color: "#414142" }}>Skills</Text>

            {data?.skills?.map((detail, index) => (
              <View style={{ flexDirection: "column", gap: 8 }}>
                <Text style={{ fontWeight: 400, fontSize: 12, color: "#414142" }}>{detail.skill}</Text>
          ...
          <View style={{ flexDirection: "column", gap: 12 }}>
            ...
            <Text style={{ fontWeight: 400, fontSize: 16, color: "#414142" }}>HOBBIES</Text>

            {data?.hobbies?.map((detail, index) => (
              <View style={{ flexDirection: "column", gap: 8 }}>
                <Text style={{ fontWeight: 400, fontSize: 12, color: "#414142" }}>{detail.title}</Text>
        ...
        <View style={{ width: 323, display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <View>
            <Text style={{ fontSize: 42, fontWeight: 400, color: "#414042" }}>{data.firstName}</Text>
            <Text style={{ fontSize: 42, fontWeight: 400, color: "#414042" }}>{data.lastName}</Text>
            <Text style={{ fontSize: 12, fontWeight: 400, color: "#414042" }}>{data.designation}</Text>
          </View>

          <View style={{ flexDirection: "column", gap: 5 }}>
            <Text style={{ fontSize: 14, fontWeight: 400, color: "#414042" }}>Hello, I'm {data.firstName}</Text>
            <Text style={{ fontSize: 12, fontWeight: 400, color: "#414042" }}>{data.summery}</Text>
          </View>
          ...
          <View>
              ...
              <View style={{ flexDirection: "column", gap: 16 }}>
                ...
                <View>
                  <Text style={{ fontWeight: 400, fontSize: 16, color: "#414042" }}>EXPERIENCE</Text>
                </View>
                {data?.experience?.map((detail, index) => (
                <View style={{ flexDirection: "column", gap: 6 }}>
                  <Text style={{ fontWeight: 700, fontSize: 12, color: "#58595B" }}>{detail.designation}</Text>
                  <Text style={{ fontWeight: 700, fontSize: 10, color: "#58595B" }}>{detail.organization}</Text>
                  <Text style={{ fontWeight: 400, fontSize: 10, color: "#58595B" }}>{detail.duration?.start?.year}-{detail.currentlyWorking ? "Present" : detail.duration?.end?.year}</Text>
                  <Text style={{ fontWeight: 700, fontSize: 10, color: "#58595B" }}>{detail.description}</Text>
                </View>
   ))}
              </View>
