      ...
      <View style={{ width: 595, display: "flex", flexDirection: "row", }}>
        ...
        <View style={{ width: 207, padding: 42, flexDirection: "column", gap: 34, backgroundColor:selectedColor, minHeight: 841 }}>
          ...
          <View style={{ flexDirection: "column", gap: 16, alignItems: "center" }}>
            <Text style={{  fontFamily: `${selectedFont} 400`, fontSize: 14, color: "#2D3033" }}>Profile</Text>
            <Text style={{ fontFamily: `${selectedFont} 400`, fontSize: 10, color: "#6D6E71" }}>{data.summery ? <>{data.summery}</> : <>About</>}</Text>
          </View>
        ...
        <View style={{ width: 388, padding: 42, display: "flex", flexDirection: "column", gap: 44 }}>
          ...
          <View style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <Text style={{ fontFamily: `${selectedFont} 400`, fontSize: 40, color: "#0D0D0D" }}>{data.firstName ? <>{data.firstName}</> : <>First Name</>}{data.lastName ? <>{data.lastName}</> : <>Last Name</>}</Text>
            <View>
          ...
          <View style={{ display: "flex", flexDirection: "column", gap: 15 }}>
            ...
            {data?.course?.map((detail, index) => (
              <View>
                <Text style={{ fontFamily: `${selectedFont} 400`, fontSize: 13, color: "#2D3033" }}>{detail.courseName} / {detail.duration?.start?.year}-{detail.duration?.end?.year}</Text>
                <Text style={{ fontFamily: `${selectedFont} 400`, fontSize: 12, color: "#6D6E71" }}>{detail.issuedBy}</Text>
              </View>
            ))}
