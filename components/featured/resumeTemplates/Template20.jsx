    ...
    <Page size='A4' style={{ padding: 24 }}>
      <View style={{ backgroundColor: '#FFF', gap: 24, display: 'flex', flexDirection: 'column', }}>
        ...
        <View style={{ display: 'flex', flexDirection: 'row', gap: 52, alignItems: 'center' }}>
          ...
          <View style={{ width: 142, display: 'flex', flexDirection: 'column', gap: 5 }}>
            <View style={{ display: 'flex', flexDirection: 'column' }}>
              <Text style={{ fontSize: 28, fontWeight: 700, color: '#303030' }}>{data.firstName}</Text>
              <Text style={{ fontSize: 28, fontWeight: 700, color: '#303030' }}>{data.lastName}</Text>
            </View>
            <Text style={{ fontSize: 10, fontWeight: 500, color: '#9B9B9B' }}>{data.designation}</Text>
          </View>
          ...
          <View style={{ width: 124, }}>
            <View style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <Text style={{ fontSize: 12, fontWeight: 500, color: '#282627' }}>CONTACT</Text>
              ...
              {data?.mobileNumber && (
                <View style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: 10, fontWeight: 600, color: '#282627' }}>{data.mobileNumber}</Text>
                </View>
              )}
              ...
              {data?.email?.length > 0 && (
                <View style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: 10, fontWeight: 600, color: '#282627' }}>{data.email}</Text>
                </View>
              )}
              ...
              {data?.location?.length > 0 && (
                <View style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: 10, fontWeight: 600, color: '#282627' }}>{data.location}</Text>
                </View>
              )}
        ...
        <View style={{ borderTopColor: '#C7C6C5', borderTop: 1, borderBottomColor: '#C7C6C5', padding: '10px 28px', borderBottom: 1, }}>
          <Text style={{ padding: 10, fontSize: 10, fontWeight: 400, color: '#6D6E71', textAlign: 'center', lineHeight: 1 }}>{data.summery}  </Text>
        </View>
        ...
        <View style={{ display: 'flex', flexDirection: 'row' }}>
          ...
          <View style={{ width: 140, paddingRight: 10, gap: 18 }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12, fontWeight: 500, color: '#282627' }}>EDUCATION</Text>
              ...
              <View style={{ display: 'flex', flexDirection: 'row', gap: 5 }}>
                ...
                {data?.education?.map((detail, index) => (
                  <View style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <Text style={{ fontSize: 10, fontWeight: 700, color: '#6D6E71', marginBottom: 2 }}>{detail.duration?.start?.year}-{detail.duration?.end?.year}</Text>
                    <Text style={{ fontSize: 10, fontWeight: 600, color: '#282627' }}>{detail.qualification} </Text>
                    <Text style={{ fontSize: 10, fontWeight: 400, color: '#6D6E71' }}> {detail.specialization}</Text>
                    <Text style={{ fontSize: 10, fontWeight: 400, color: '#6D6E71' }}>{detail.instituteName}</Text>
                  </View>
                ))}
              </View>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12, fontWeight: 500, color: '#282627' }}>HOBBBE</Text>
            ...
            {data?.languages?.length > 0 && (
              <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <Text style={{ color: '#292627', fontSize: 12, fontWeight: 500 }}>LANGUAGE</Text>
                {data.languages?.map((detail, index) => {
                  ...
                  return (
                    <View style={{ width: 130, display: 'flex', flexDirection: 'row', alignItems: "center" }}>
                      <Text style={{ width: 55, fontSize: 10, fontWeight: 400, color: '#6D6E71' }}>{detail.languages}</Text>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12, fontWeight: 500, color: '#282627' }}>AWARDS</Text>
              ...
              <View style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'flex-start' }}>
                ...
                <View style={{ display: 'flex', flexDirection: 'column', gap: 5, maxWidth: '90%', }}>
                  <Text style={{ fontSize: 10, fontWeight: 700, color: '#282627' }}>WEB DEVELOPER OF THE YEAR</Text>
                  <Text style={{ fontSize: 10, fontWeight: 400, color: '#8F9193' }}>Abc Creative Studio / 2017</Text>
          ...
          <View style={{ width: 236, paddingHorizontal: 10, borderLeft: 1, borderLeftColor: '#C7C6C5', borderRight: 1, borderRightColor: '#C7C6C5' }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12, fontWeight: 500, color: '#282627' }}>EXPERIENCE</Text>
              {data.experience?.map((detail, index) => (
                <View style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <Text style={{ fontWeight: 700, fontSize: 10, color: '#2C363D' }}>{detail.designation} </Text>
                  <Text style={{ fontWeight: 600, fontSize: 10, color: '#939598' }}>{detail.duration?.start?.year}-{" "}{detail.currentlyWorking ? "Present" : detail.duration?.end?.year}</Text>
                  <Text style={{ fontWeight: 600, fontSize: 10, color: '#6D6E71' }}>{detail.description} </Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={{ width: 142, paddingLeft: 10 }}>
            <View style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              {data?.languages?.length > 0 && (
                <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <Text style={{ color: '#292627', fontSize: 12, fontWeight: 500 }}>SOFTWARE</Text>
                  {data.languages?.map((detail, index) => {
                    const calculateWidthPercentage = (rating) => {
                      let zerosCount = 0;
                      if (rating && rating.length > 0) {
              ...
              <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <Text style={{ fontSize: 12, fontWeight: 500, color: '#282627' }}>SKILLS</Text>
              ...
              {data?.languages?.length > 0 && (
                <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <Text style={{ color: '#292627', fontSize: 12, fontWeight: 500 }}>KNOWLEDGE</Text>
                  {data.languages?.map((detail, index) => {
                    const calculateWidthPercentage = (rating) => {
                      let zerosCount = 0;
                      if (rating && rating.length > 0) {
