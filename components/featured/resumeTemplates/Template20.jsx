    ...
    <Page size='A4' style={{ padding: 24 }}>
      <View style={{ backgroundColor: '#FFF', gap: 24, display: 'flex', flexDirection: 'column', }}>
        ...
        <View style={{ display: 'flex', flexDirection: 'row', gap: 52, alignItems: 'center' }}>
          ...
          <View style={{ width: 142, display: 'flex', flexDirection: 'column', gap: 5 }}>
            <View style={{ display: 'flex', flexDirection: 'column' }}>
              <Text style={{ fontSize: 28, fontFamily:'Montserrat 700', color: '#303030' }}>{data.firstName}</Text>
              <Text style={{ fontSize: 28, fontFamily:'Montserrat 700', color: '#303030' }}>{data.lastName}</Text>
            </View>
            <Text style={{ fontSize: 10,fontFamily:'Montserrat 500', color: '#9B9B9B' }}>{data.designation}</Text>
          </View>
          ...
          <View style={{ width: 124, }}>
            <View style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <Text style={{ fontSize: 12,fontFamily:'Montserrat 500', color: '#282627' }}>CONTACT</Text>
        ...
        <View style={{ borderTopColor: '#C7C6C5', borderTop: 1, borderBottomColor: '#C7C6C5', padding: '10px 28px', borderBottom: 1, }}>
          <Text style={{ padding: 10, fontSize: 10,fontFamily:'Montserrat 400', color: '#6D6E71', textAlign: 'center', lineHeight: 1 }}>{data.summery}  </Text>
        </View>
        ...
        <View style={{ display: 'flex', flexDirection: 'row' }}>
          ...
          <View style={{ width: 140, paddingRight: 10, gap: 18 }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12,fontFamily:'Montserrat 500', color: '#282627' }}>EDUCATION</Text>
              ...
              <View style={{ display: 'flex', flexDirection: 'row', gap: 5 }}>
                ...
                {data?.education?.map((detail, index) => (
                  <View style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <Text style={{ fontSize: 10,fontFamily:'Montserrat 700', color: '#6D6E71', marginBottom: 2 }}>{detail.duration?.start?.year}-{detail.duration?.end?.year}</Text>
                    <Text style={{ fontSize: 10,fontFamily:'Montserrat 600', color: '#282627' }}>{detail.qualification} </Text>
                    <Text style={{ fontSize: 10,fontFamily:'Montserrat 400', color: '#6D6E71' }}> {detail.specialization}</Text>
                    <Text style={{ fontSize: 10,fontFamily:'Montserrat 400', color: '#6D6E71' }}>{detail.instituteName}</Text>
                  </View>
                ))}
              </View>
            ...
            {/* <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12,fontFamily:'Montserrat 500', color: '#282627' }}>HOBBBE</Text>
            ...
            {data?.languages?.length > 0 && (
              <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <Text style={{ color: '#292627', fontSize: 12, fontFamily:'Montserrat 500'}}>LANGUAGE</Text>
                {data.languages?.map((detail, index) => {
                  ...
                  return (
                    <View style={{ width: 130, display: 'flex', flexDirection: 'row', alignItems: "center" }}>
                      <Text style={{ width: 55, fontSize: 10,fontFamily:'Montserrat 400', color: '#6D6E71' }}>{detail.languages}</Text>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Text style={{ fontSize: 12,fontFamily:'Montserrat 500', color: '#282627' }}>AWARDS</Text>
              ...
              <View style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'flex-start' }}>
                ...
                <View style={{ display: 'flex', flexDirection: 'column', gap: 5, maxWidth: '90%', }}>
                  <Text style={{ fontSize: 10,fontFamily:'Montserrat 700', color: '#282627' }}>WEB DEVELOPER OF THE YEAR</Text>
                  <Text style={{ fontSize: 10,fontFamily:'Montserrat 400', color: '#8F9193' }}>Abc Creative Studio / 2017</Text>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {data.experience?.map((detail, index) => (
                <View style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <Text style={{ fontFamily:'Montserrat 700', fontSize: 10, color: '#2C363D' }}>{detail.designation} </Text>
                  <Text style={{ fontFamily:'Montserrat 600', fontSize: 10, color: '#939598' }}>{detail.duration?.start?.year}-{" "}{detail.currentlyWorking ? "Present" : detail.duration?.end?.year}</Text>
                  <Text style={{ fontFamily:'Montserrat 400', fontSize: 10, color: '#6D6E71' }}>{detail.description} </Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={{ width: 142, paddingLeft: 10 }}>
            <View style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              {data?.languages?.length > 0 && (
                <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <Text style={{ color: '#292627', fontSize: 12,fontFamily:'Montserrat 500'}}>SOFTWARE</Text>
                  {data.languages?.map((detail, index) => {
                    const calculateWidthPercentage = (rating) => {
                      let zerosCount = 0;
                      if (rating && rating.length > 0) {
              ...
              <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <Text style={{ fontSize: 12, fontFamily:'Montserrat 500', color: '#282627' }}>SKILLS</Text>

                <View style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                  <Svg width="6" height="7" viewBox="0 0 6 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <Path d="M3 6.5C4.65685 6.5 6 5.15685 6 3.5C6 1.84315 4.65685 0.5 3 0.5C1.34315 0.5 0 1.84315 0 3.5C0 5.15685 1.34315 6.5 3 6.5Z" fill="#5E5F5E" />
                  </Svg>
                  <Text style={{ fontSize: 10, fontFamily:'Montserrat 400', color: '#6D6E71' }}>CYCLING</Text>
                </View>
              </View>

              {data?.languages?.length > 0 && (
                <View style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <Text style={{ color: '#292627', fontSize: 12, fontFamily:'Montserrat 500' }}>KNOWLEDGE</Text>
                  {data.languages?.map((detail, index) => {
                    const calculateWidthPercentage = (rating) => {
                      let zerosCount = 0;
                      if (rating && rating.length > 0) {
