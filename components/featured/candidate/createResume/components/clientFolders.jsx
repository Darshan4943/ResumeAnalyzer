...
function ClientFolders({ toggleSelect, isList, tabIndex, setTabIndex, data, setData, setFolderData, files, setFiles, clientData, setClientData, tab, select, setSelect, selectedIndexes, setSelectedIndexes, openClientFolder }) {
  ...
  useEffect(() => {
    if (files && Array.isArray(files) && files.length > 0) {
      axios.head(files[0].resumeUrl)
        .then(response => {
          const contentLengthBytes = parseInt(response.headers['content-length'], 10);
              ...
              {!isList ?
                clientData?.map((item, index) => (
                  <>
                    ...
                    <div onClick={() => { !select && openClientFolder(index) }} key={index} className=' break-all flex flex-col items-center text-center cursor-pointer h-fit w-[120px] px-2'>
                ...
                <table className="w-[100%]" >
                  ...
                  <tbody >
                    {clientData.map((item, index) => (
                      <tr key={index} onClick={() => openClientFolder(index)} className=''>
                        <td className='px-4 py-2 font-medium flex gap-2 relative items-center cursor-pointer '>
                          ...
                          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M21.4286 4.49999H12.5299L11.5263 2.45093C11.3138 2.01394 10.9857 1.64642 10.5792 1.38999C10.1727 1.13355 9.70391 0.998462 9.22603 1.00001H2.57143C1.88967 1.00078 1.23606 1.27758 0.753981 1.7697C0.271907 2.26181 0.000748696 2.92904 0 3.625V19.3749C0.000748696 20.0709 0.271907 20.7381 0.753981 21.2302C1.23606 21.7223 1.88967 21.9991 2.57143 21.9999H21.4286C22.1103 21.9991 22.7639 21.7223 23.246 21.2302C23.7281 20.7381 23.9993 20.0709 24 19.3749V7.12497C23.9993 6.42902 23.7281 5.76179 23.246 5.26968C22.7639 4.77756 22.1103 4.50076 21.4286 4.49999Z" fill="#4294FF" />
                            <path d="M22.0543 21.9122C21.851 21.9725 21.6402 22.0019 21.4286 21.9997H2.57143C1.89006 21.9977 1.23716 21.7205 0.755352 21.2286C0.273547 20.7368 0.00199013 20.0703 1.77688e-07 19.3747V3.62483C-0.000193566 3.09445 0.158055 2.57663 0.45363 2.14045C0.749206 1.70427 1.16805 1.37047 1.65429 1.18359C4.05429 8.0523 9.66857 18.5085 22.0543 21.9122Z" fill="#2965ED" />
                          </svg>

                          {item.firstName} {item.lastName}
                        </td>
                        <td className='px-4 py-2 text-[#858585]'>{formatDate(item?.updatedAt)}</td>
