  ...
  const addJobView = async () => {
    try {
      const response = await axios.post(
        `https://api.skilotech.com/api/jobs/views/${selectedJob?._id}`
      );
            ...
            >
              ...
              {/* {selectedJob.requiredQualification && (
                <div className="flex flex-col gap-[10px] pb-[6px]">
                  <div className="text-[16px] font-[600]">Qualifications : <span className="text-[14px] font-[400]">  {selectedJob.requiredQualification} </span></div>
                ...
                <button
                  ...
                  onClick={() => {
                    if (!selectedJob?.isExternal) {
                      ...
                      window.open(selectedJob?.jobLink, "_blank", "noopener,noreferrer")
                    }
                  }}
