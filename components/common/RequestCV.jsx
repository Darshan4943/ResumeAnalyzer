...
function RequestCV({ isCandidate, skilotechCollection }) {
  ...
  const getUserFolders = async () => {
    try {
      const response = await axios.get(`http://localhost:2000/api/folder/getFolder/${userDataGlobal?._id}`);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:2000/api/candidates/getDistinctCandidateAttributes")
      .then((res) => {
